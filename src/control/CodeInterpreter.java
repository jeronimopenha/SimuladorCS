...
public class CodeInterpreter {
    ...
    /**
     * Função responsável por interpretar o código do Computador Simplificado e
     * executá-lo.
     *
     * @param dataMap -
     ...
     */
    public void execute(HashMap<Integer, JLabel> dataMap, List<Integer> cardList, JTextArea jtxa) {
        //try {
        int counter = 1;
        boolean stop = false;
        try {
            while (!stop) {
                ...
                Instruction inst = this.instructions.get(counter);
                switch (inst.getInst()) {
                    case PGC:
                        if (cardList.isEmpty()) {
